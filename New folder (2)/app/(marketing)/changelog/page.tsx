...
function TimelineEntry({ entry, index }: { entry: (typeof entries)[number]; index: number }) {
  return (
    ...
    >
      ...
      >
        <motion.span
          className="rounded-md bg-secondary px-2.5 py-1 font-mono text-[12px] font-medium text-muted-foreground"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true, margin: "-60px" }}
      ...
      {/* Title */}
      <motion.h2
        className="text-[1.25rem] font-semibold leading-snug tracking-[-0.02em] text-foreground md:text-[1.5rem]"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true, margin: "-60px" }}
    ...
    <div className="bg-background">
      ...
      <section className="border-t border-border/40 py-24 md:py-32">
        <div className="max-w-[800px] mx-auto px-6">
          <div className="relative border-l border-border/60 pl-8">
            {entries.map((entry, i) => (
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-40px" }}
            transition={{ duration: 0.6, ease }}
            className="mt-20 flex flex-col items-start gap-3 border-t border-border/40 pt-12 sm:flex-row sm:items-center sm:justify-between"
          >
            <p className="text-[14px] text-muted-foreground">
              Want early access to upcoming features?{" "}
              <span className="text-foreground font-medium">Join the beta list.</span>
            </p>
            <Link href="/register" className="cursor-pointer group inline-flex w-full sm:w-auto h-10 items-center justify-center gap-2 rounded-lg bg-foreground px-5 text-[13px] font-medium text-background transition-all duration-200 hover:bg-foreground/90">
                Start free trial
