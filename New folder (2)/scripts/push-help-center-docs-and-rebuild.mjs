#!/usr/bin/env node
/**
 * Push settings/integrations + channels/[type] (Docs → replyma.com/help-center/article) to server, rebuild frontend, restart PM2.
 */
...
const files = [
  ['app/(dashboard)/settings/integrations/page.tsx', 'app/(dashboard)/settings/integrations/page.tsx'],
  ['app/(dashboard)/channels/[type]/page.tsx', 'app/(dashboard)/channels/[type]/page.tsx'],
];
...
conn.on('ready', async () => {
  try {
    for (const [localRel, remoteRel] of files) {
      const local = path.join(localRoot, localRel);
      const remoteDir = `${r}/${path.dirname(remoteRel).replace(/\\/g, '/')}`;
      const remote = `${r}/${remoteRel.replace(/\\/g, '/')}`;
      if (!fs.existsSync(local)) throw new Error('Not found: ' + localRel);
      await run(conn, `mkdir -p '${remoteDir}'`);
      await putFile(conn, local, remote);
      console.log('Synced', localRel);
    }
    ...
    console.log('Done. Docs links now point to https://replyma.com/help-center/<article>.');
  } catch (e) {
