#!/usr/bin/env node
/**
 * Push channels [type] page (live chat theme presets) to server, rebuild frontend, restart PM2.
 */
...
const file = 'app/(dashboard)/channels/[type]/page.tsx';
...
conn.on('ready', async () => {
  try {
    ...
    if (!fs.existsSync(local)) throw new Error('Not found: ' + file);
    ...
    console.log('Done. Live chat theme presets updated on server.');
  } catch (e) {
