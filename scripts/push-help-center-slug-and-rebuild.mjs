#!/usr/bin/env node
/**
 * Push help-center [slug] page (top padding) to server, rebuild frontend, restart PM2.
 */
...
const fileRel = 'app/(marketing)/help-center/[slug]/page.tsx';

const conn = new Client();
conn.on('ready', async () => {
  try {
    const local = path.join(localRoot, fileRel);
    const remoteDir = `${r}/${path.dirname(fileRel).replace(/\\/g, '/')}`;
    const remote = `${r}/${fileRel.replace(/\\/g, '/')}`;
    if (!fs.existsSync(local)) throw new Error('Not found: ' + fileRel);
    await run(conn, `mkdir -p '${remoteDir}'`);
    await putFile(conn, local, remote);
    console.log('Synced', fileRel);
    ...
    console.log('Done. Help center article page updated.');
  } catch (e) {
